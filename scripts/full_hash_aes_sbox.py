...
input_file_path = '../tb/test_files/message.txt'
target_file_path = '../tb/test_files/target.txt'
...
def byte_list(C: int) -> list:
    return [int(byte) for byte in C.to_bytes(8, 'big')]


def hex_values(H: list) -> list:
    return [hex(byte)[2:].upper() if len(hex(byte)) == 4 else '0' + hex(byte)[2:].upper() for byte in H]


def write_to_file(M, length, digest_):
    with open(input_file_path, 'w') as file:
        file.write(str(length) + "\n")
        file.write(M)

    with open(target_file_path, 'w') as file:
        file.write(digest_.lower())


def hash_main_round(m, H):
    H_temp = []
    for i in range(8):
        temp = (H[(i + 1) % 8] ^ m)
        temp = (temp << i) & 0xFF | (temp >> (8 - i))
        H_temp.insert(i, aes_sbox[row(temp)][col(temp)])
    return H_temp


def hash_final_round(C, H):
    H_temp = []
    for i in range(8):
        temp = (H[(i + 1) % 8] ^ C[i])
        temp = (temp << i) & 0xFF | (temp >> (8 - i))
...
def aes_full_hash_sbox(M, H_init):
    msg_len = len(M)
    C = byte_list(msg_len)
    H = H_init
    for m in M:
...
if __name__ == "__main__":
    M = input('Enter the message you want to hash: ')
    digest = aes_full_hash_sbox(M, H_init)
    human_readable_digest = ''.join(hex_values(digest))
    print(f'Input message:     "{M}"')
    print(f'Message length:    {len(M)}')
    print(f'Computed digest:   {human_readable_digest}')
    if len(sys.argv) >= 2 and (sys.argv[1] == '-w'):
        write_to_file(M, len(M), human_readable_digest)
        print(f'Wrote files to:    {input_file_path}, {target_file_path}.')
